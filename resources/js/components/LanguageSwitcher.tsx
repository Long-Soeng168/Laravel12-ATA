            ...
            {tabs.map(({ value, icon, label }) => (
                <a href={`/lang/${value}`}>
                    <button
                        key={value}
                        className={cn(
                            'flex items-center gap-2 rounded-none px-1.5 py-1.5 transition-colors',
                            currentLocale === value
                                ? 'bg-white shadow dark:bg-neutral-700 dark:text-neutral-100'
                                : 'text-neutral-500 hover:bg-neutral-200/60 hover:text-black dark:text-neutral-400 dark:hover:bg-neutral-700/60',
                        )}
                    >
                        {icon && <img src={icon} alt="" className={`aspect-square w-4 rounded-full object-contain lg:w-4`} />}
