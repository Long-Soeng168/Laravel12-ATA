...
export default function CreateForm() {
    ...
    const { editData, readOnly, owners, is_user_create_or_edit_garage, provinces } = usePage<any>().props;
    ...
    const { data, setData, post, processing, transform, progress, errors, reset } = useForm({
        is_user_create_or_edit_garage: is_user_create_or_edit_garage || false,
    ...
    const onSubmit = (e: React.FormEvent) => {
        ...
        // Added 'data' parameter here to properly map Inertia's form state
        transform((data) => ({
            ...data,
        ...
        const url = editData?.id ? `/admin/garages/${editData.id}/update` : '/admin/garages';

        post(url, {
            preserveScroll: false,
            onSuccess: (page: any) => {
                // Only reset form and file states if we are creating a new record
                if (!editData?.id) {
                    reset();
                    setLogoFiles(null);
                    setBannerFiles(null);
                ...
                <div className="form-field-container">
                    {provinces?.length > 0 && (
                        <FormCombobox
                            ...
                            className={`${!(owners?.length > 0) && 'col-span-2'}`}
                        />
                    )}
                    {owners?.length > 0 && (
                        <FormField
