                    ...
                    <TableBody className="table-body rounded-md">
                        {tableData?.data?.map((item: any, index: number) => (
                            <TableRow
                                className="table-row"
                                key={item.id}
                                // onDoubleClick={() => router.visit(`/admin/item-body-types?category_code=${item.code}`)}
                            >
                                ...
                                <TableCellActions>
                                    {item.deleted_at ? (
                                        <RecoverItem
                                            ...
                                            recoverPath={`/admin/item-body-types/${item.id}/recover`}
                                            permission="user update"
                                        />
                                    ) : (
                                        <div className="space-x-2">
                                            {/* Edit Dialog */}
                                            <EditItemButton url={`/admin/item-body-types/${item.id}/edit`} permission="item update" />

                                            {/* View Dialog */}
                                            <ViewItemButton url={`/admin/item-body-types/${item.id}`} permission="item view" />

                                            {/* Delete Item */}
                                            <DeleteItemButton deletePath="/admin/item-body-types/" id={item.id} permission="item delete" />
                                        </div>
                                    )}
                                </TableCellActions>
                                <TableCell>
                                    <TableCellAvatar
                                        ...
                                        alt={item.name}
                                        image={`/assets/images/item_body_types/thumb/${item.image}`}
                                    />
                                </TableCell>
