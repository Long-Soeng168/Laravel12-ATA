                    ...
                    <TableBody className="table-body rounded-md">
                        {tableData?.data?.map((item: any, index: number) => (
                            <TableRow
                                className="table-row"
                                key={item.id}
                                // onDoubleClick={() => router.visit(`/admin/provinces?category_code=${item.code}`)}
                            >
                                ...
                                <TableCellActions>
                                    {item.deleted_at ? (
                                        <RecoverItem
                                            ...
                                            recoverPath={`/admin/provinces/${item.id}/recover`}
                                            permission="user update"
                                        />
                                    ) : (
                                        <div className="space-x-2">
                                            {/* Edit Dialog */}
                                            <EditItemButton url={`/admin/provinces/${item.id}/edit`} permission="item update" />

                                            {/* View Dialog */}
                                            <ViewItemButton url={`/admin/provinces/${item.id}`} permission="item view" />

                                            {/* Delete Item */}
                                            <DeleteItemButton deletePath="/admin/provinces/" id={item.id} permission="item delete" />
                                        </div>
                                    )}
                                </TableCellActions>
                                {/* <TableCell>
