...
const VideoViewCount = () => {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        {
            title: currentLocale === 'kh' ? 'វីដេអូ' : 'Videos',
            href: '/admin/videos',
        },
        {
            title: currentLocale === 'kh' ? 'ចំនួនអ្នកមើល' : 'View Counts',
            href: '/admin/video_view_counts',
        },
    ];
    const urlParams = new URLSearchParams(window.location.search);
    const { get, transform } = inertiaUseForm();

    const { totalViews, dailyTrends, topVideo, from_date, to_date } = usePage().props as any;
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex flex-col">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            {currentLocale === 'kh' ? 'វិភាគចំនួនអ្នកចូលមើលវីដេអូរបស់អ្នក' : 'Analyze the performance and traffic of your videos.'}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        />
                        <form method="GET" action="/admin/video_view_counts/export" target="_blank">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <Card className="md:col-span-1 lg:col-span-2">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-sm font-medium">{currentLocale === 'kh' ? 'វីដេអូដែលមានអ្នកមើលច្រើនជាងគេ' : 'Most Viewed Video'}</CardTitle>
                                ...
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                    <div>
                                        <div className="text-xl font-bold truncate max-w-sm">
                                            {currentLocale === 'kh' ? (topVideo.video?.title_kh || topVideo.video?.title) : topVideo.video?.title}
                                        </div>
                                        <p className="text-xs text-muted-foreground mt-1">
                                            ID: {topVideo.video_id}
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-2 bg-primary/10 px-3 py-1 rounded-full">
                                        <TrendingUp className="h-4 w-4 text-primary" />
                                        <span className="text-sm font-semibold text-primary">{topVideo.total_views} {currentLocale === 'kh' ? 'អ្នកមើល' : 'Views'}</span>
                                    </div>
                                </div>
                            ) : (
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between pb-2 border-b mb-4">
                        <CardTitle>{currentLocale === 'kh' ? 'ចំណាត់ថ្នាក់វីដេអូ' : 'Video Ranking'}</CardTitle>
