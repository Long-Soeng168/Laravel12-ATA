...
const defaultCenter = {
    ...
    // Centered around Siem Reap, Cambodia
    // lat: 13.3618,
    // lng: 103.8606,
};
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    setMapCenter(userPos); // center map on user
                    setUserLocation(userPos); // show marker on user
                },
                (error) => {
                    console.warn('Geolocation error:', error.message);
                    // fallback to default center (already set)
                },
            );
        }
    }, [isLoaded]);
                ...
                <div className="flex-1">
                    {!isLoaded ? (
                        ...
                        <GoogleMap
                            ...
                            center={mapCenter} // You might want to dynamically calculate this based on your markers
                        ...
                        >
                            {userLocation && (
                                <Marker
                                    ...
                                    icon={{
                                        url: '/assets/icons/user-pin.png',
                                        scaledSize: new window.google.maps.Size(60, 60), // Resize marker
                                        anchor: new window.google.maps.Point(50, 60), // Position anchor
                                    }}
                                    title="You are here"
                                >
                                    {showUserInfo && (
                            ...
                            {locationsData.map((location) => (
                                <Marker
                                    ...
                                    icon={{
                                        url: '/assets/icons/garage-pin2.png', // Custom marker icon (replace with your own)
                                        scaledSize: new window.google.maps.Size(60, 60), // Resize marker
                                        anchor: new window.google.maps.Point(50, 60), // Position anchor
                                    }}
                                >
