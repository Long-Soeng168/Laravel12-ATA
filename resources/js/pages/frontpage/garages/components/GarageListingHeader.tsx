...
import { ArrowDownUp, Check, ChevronDown, ChevronRightIcon, MapPin, Search, ShieldCheck, X } from 'lucide-react';
...
export default function GarageListingHeader() {
    ...
    const updateFilter = (key: string, value: string) => {
        ...
        // Reset pagination to page 1 whenever a filter (like search query) changes
    ...
    const hasActiveFilters = Object.keys(filters).length > 0 && !(Object.keys(filters).length === 1 && filters.page);
    const hideScrollbar = '[&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]';

    const breadcrumbs = [
        { label: t('Home'), action: () => router.get('/') },
        { label: t('Garages'), action: clearFilters },
    ];
                ...
                <form onSubmit={handleSearchSubmit} className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
                    <div className="relative flex-1">
                        ...
                        <Input
                            ...
                            placeholder={t('Search by garage name, or address...')}
                        />
                    </div>
                    <button
                        type="submit"
