...
const MapFloatingButton = () => {
    ...
    return (
        <div className="fixed bottom-[85px] left-4 z-50 sm:bottom-6 sm:left-6">
            <a
                href="/garages_map"
                aria-label={currentLocale === 'kh' ? 'ផែនទីយានដ្ឋាន' : 'Garages Map'}
                className="group flex items-center gap-2 rounded-full border border-blue-400/20 bg-blue-500 py-3 pr-5 pl-4 text-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] active:scale-95 sm:gap-3 sm:py-3.5"
            >
                ...
                <span className="text-xs font-medium sm:text-sm">{currentLocale === 'kh' ? 'ផែនទីយានដ្ឋាន' : 'Garages Map'}</span>
