...
class UserShopController extends Controller implements HasMiddleware
{
    ...
    public function edit()
    {
        $all_users = User::orderBy('id', 'desc')
            ->where('shop_id', null)
            ->get();
        $admin_shop = Shop::findOrFail(Auth::user()->shop_id);
        if ($admin_shop->id != Auth::user()->shop_id) {
            abort(404);
        }
        // return ($all_users);
        return Inertia::render('user-dashboard/shops/Create', [
            'editData' => $admin_shop->load('owner'),
            'all_users' => $all_users,
        ]);
    }
     ...
     */
    public function update(Request $request, Shop $admin_shop)
    {
        if ($admin_shop->id != Auth::user()->shop_id) {
            abort(404);
        }
        ...
        if ($validated['owner_user_id'] != $admin_shop->owner_user_id) {
            User::where('id', $admin_shop->owner_user_id)->update([
                'shop_id' => null,
            ]);
        }
        ...
        if ($image_file) {
            try {
                ...
                if ($admin_shop->logo && $created_image_name) {
                    ImageHelper::deleteImage($admin_shop->logo, 'assets/images/shops');
                }
            } catch (\Exception $e) {
        ...
        if ($banner_file) {
            try {
                ...
                if ($admin_shop->banner && $created_image_name) {
                    ImageHelper::deleteImage($admin_shop->banner, 'assets/images/shops');
                }
            } catch (\Exception $e) {
                return redirect()->back()->with('error', 'Failed to upload image: ' . $e->getMessage());
            }
    ...
    public function update_status(Request $request, Shop $admin_shop)
    {
        $request->validate([
     ...
     */
    public function destroy(Shop $admin_shop)
    {
        if ($admin_shop->logo) {
            ImageHelper::deleteImage($admin_shop->logo, 'assets/images/shops');
        }
        if ($admin_shop->banner) {
            ImageHelper::deleteImage($admin_shop->banner, 'assets/images/shops');
        }
        $admin_shop->delete();
